...
var projectionMutator = d3.geo.projectionMutator;
...
function gingery(ρ, n) {
  ...
  function forward(λ, φ) {
    ...
    if (r2 > ρ2) {
      var r = Math.sqrt(r2),
          ...
          s_ = arcLength_(α, k_),
          e = (π - ρ) / integrate(s_, ρcosα, π, N);

      x = r;
      var i = 50, δ;
      do {
        x -= δ = (ρ + integrate(s_, ρcosα, x, N) * e - r) / (s_(x) * e);
  ...
  forward.invert = function(x, y) {
    ...
    if (r2 > ρ2) {
      ...
      r = ρ + integrate(s_, ρcosα, x, N) * (π - ρ) / integrate(s_, ρcosα, π, N);
      θ = θ0 + α;
      x = r * Math.cos(θ);
      y = r * Math.sin(θ);
    }
